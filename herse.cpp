...
void pertechargeherse::saveDataWrapper() {
    QString fileName = QFileDialog::getSaveFileName(
            this, "Sauvegarder", QDir::homePath(), "(*.dat)");
...
void pertechargeherse::loadDataWrapper() {
    QString fileName = QFileDialog::getOpenFileName(
            this, "Charger un fichier", QDir::homePath(), "(*.dat)");
...
void pertechargeherse::importdonees(float debitLH, float diametre, float interval, std::vector<float> &data,
                                    float hauteurligne) {
        ...
        if (indices[i] != -1) { // Si l'indice n'est pas égal à -1, on remplit la case correspondante avec la donnée.
            if (i == 0) {
                lineEdit->setText(QString::number(
                        static_cast<int>(rowData[i]))); // Formatte la donnée en nombre entier s'il s'agit de la première colonne.
            } else {
                lineEdit->setText(
                ...
                if (keyEvent->key() == Qt::Key_Tab) {
                    ...
                    for (i = 0; i < _Donnees.size() && !found; ++i) {
                        for (j = 0; j < _Donnees[i].size() && !found; ++j) {
                            QLineEdit *currentLineEdit = qobject_cast<QLineEdit *>(
                                    gridLayout->itemAtPosition(i + 1, j)->widget());
                    ...
                    if (found) {
                        ...
                        if (i + 1 < _Donnees.size()) {
                            QLineEdit *nextLineEdit = qobject_cast<QLineEdit *>(
                                    gridLayout->itemAtPosition(i + 2, j - 1)->widget());
                            if (nextLineEdit) {
            ...
            if (initialLineEdit && (mouseEvent->pos().y() - lastMousePos.y() > 10)) {
                ...
                if (jInitial == jCurrent && iCurrent > iInitial) {
                    for (int i = iInitial + 1; i <= iCurrent; ++i) {
                        QLineEdit *currentLineEdit = qobject_cast<QLineEdit *>(
                                gridLayout->itemAtPosition(i + 1, jInitial)->widget());
                        if (currentLineEdit) {
...
void pertechargeherse::addRow() {
    ...
    QObject::connect(okButton, &QPushButton::clicked,
                     [this, &addRowDialog, positionSpinBox, debitSpinBox, diametreSpinBox, longueurSpinBox, hauteurSpinBox]() {
